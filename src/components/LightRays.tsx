...
void main() {
  ...
  gl_FragColor = color;
      ...
      const handleResize = () => {
        ...
        if (uniformsRef.current) {
          const newW = rendererRef.current.gl.canvas.width;
          const newH = rendererRef.current.gl.canvas.height;
          uniformsRef.current.iResolution.value = [newW, newH];
          const { anchor: a, dir: d } = getAnchorAndDir(raysOrigin, newW, newH);
          uniformsRef.current.rayPos.value = a;
          uniformsRef.current.rayDir.value = d;
        }
      };
      ...
      const update = (t: number) => {
        ...
        uniformsRef.current.iTime.value = (t - startTime) * 0.001;
        if (followMouse) {
  ...
  return <div ref={containerRef} className={`lightrays-container ${className}`} />;
};
